...
def _additional_media_roots(values: Mapping[str, str]) -> tuple[MediaRoot, ...]:
    raw_value = values.get("ADDITIONAL_MEDIA_ROOTS_JSON", "").strip()
    if not raw_value:
        return ()
    try:
        items = json.loads(raw_value)
    except json.JSONDecodeError as exc:
        raise ValueError("ADDITIONAL_MEDIA_ROOTS_JSON must be valid JSON") from exc
    if not isinstance(items, list):
        raise ValueError("ADDITIONAL_MEDIA_ROOTS_JSON must be a JSON array")
    ...
    for item in items:
        ...
        if (
            ...
            raise ValueError("media root ids must be unique URL-safe identifiers")
        if not label:
...
def load_settings(env: Mapping[str, str] | None = None) -> Settings:
    ...
    settings = Settings(
        ...
        additional_media_roots=_additional_media_roots(values),
    )
